...
replaces the previous row from that plugin on live clients and fresh timeline fetches. Rows use the
same persisted canonical timeline as provider rows and retain their epoch/sequence identity across a
daemon restart. A row without an installed renderer shows an unavailable placeholder. Serialized `data`
